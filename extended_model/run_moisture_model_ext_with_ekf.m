...
P = eye(10) * 0.01;   % error covariance of the initial guess

% Kalman filter Q (model error covariance) and R (measurement error covar)
Q = eye(10) * 0.001;
R = eye(3) * 1.0;

% the observation operator is a 3 x 10 matrix with I_3 on the left
H = zeros(3, 10);
H(1:3,1:3) = eye(3);
...
for i=2:N
    ...
    m_pred = moisture_model_ext(T, q, p, m_f(i-1,:)', r(i), dt);
